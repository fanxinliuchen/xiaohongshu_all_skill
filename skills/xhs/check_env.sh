...
ENV_FILE="${XHS_ENV_FILE:-${SKILL_DIR}/.env}"
if [ -f "$ENV_FILE" ]; then
  set -a
...
echo "=== 4. 检查生图 API 配置 ==="
# 兼容 ARK_* 变量映射到 OpenAI-compatible 模式
IMG_API_TYPE="${IMG_API_TYPE:-gemini}"
IMG_API_KEY="${IMG_API_KEY:-${ARK_API_KEY:-}}"
IMG_API_BASE="${IMG_API_BASE:-${ARK_BASE_URL:-https://api.openai.com/v1}}"
IMG_MODEL="${IMG_MODEL:-${ARK_MODEL_ID:-dall-e-3}}"
IMG_SIZE="${IMG_SIZE:-${ARK_IMAGE_SIZE:-1536x1024}}"
IMG_OK=false
...
case "$IMG_API_TYPE" in
  ...
  openai)
    if [ -n "${IMG_API_KEY:-}" ]; then
      echo "✅ OpenAI 兼容 API Key 已配置 (IMG_API_TYPE=openai, BASE=${IMG_API_BASE:-https://api.openai.com/v1}, MODEL=${IMG_MODEL:-dall-e-3}, SIZE=${IMG_SIZE:-1536x1024})"
      IMG_OK=true
    else
